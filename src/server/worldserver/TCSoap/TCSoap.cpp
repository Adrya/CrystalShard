...
void TCSoapRunnable::run()
{
    ...
    {
        sLog->outError("CSSoap: couldn't bind to %s:%d", m_host.c_str(), m_port);
        exit(-1);
    }

    sLog->outString("CSSoap: bound to http://%s:%d", m_host.c_str(), m_port);
    ...
    while (!World::IsStopped())
    {
        ...
        sLog->outDebug(LOG_FILTER_NETWORKIO, "CSSoap: accepted connection from IP=%d.%d.%d.%d", (int)(soap.ip>>24)&0xFF, (int)(soap.ip>>16)&0xFF, (int)(soap.ip>>8)&0xFF, (int)soap.ip&0xFF);
...
int ns1__executeCommand(soap* soap, char* command, char** result)
{
    ...
    {
        sLog->outDebug(LOG_FILTER_NETWORKIO, "CSSoap: Client didn't provide login information");
        return 401;
    }
    ...
    if (!accountId)
    {
        sLog->outDebug(LOG_FILTER_NETWORKIO, "CSSoap: Client used invalid username '%s'", soap->userid);
        return 401;
    }

    if (!AccountMgr::CheckPassword(accountId, soap->passwd))
    {
        sLog->outDebug(LOG_FILTER_NETWORKIO, "CSSoap: invalid password for account '%s'", soap->userid);
        return 401;
    }

    if (AccountMgr::GetSecurity(accountId) < SEC_ADMINISTRATOR)
    {
        sLog->outDebug(LOG_FILTER_NETWORKIO, "CSSoap: %s's gmlevel is too low", soap->userid);
        return 403;
    }
    ...
    sLog->outDebug(LOG_FILTER_NETWORKIO, "CSSoap: got command '%s'", command);
    ...
    if (acc)
    {
        sLog->outError("CSSoap: Error while acquiring lock, acc = %i, errno = %u", acc, errno);
    }
