...
void RARunnable::run()
{
    ...
    {
        sLog->outError("CrystalShard RA can not bind to port %d on %s", raport, stringip.c_str());
        return;
    }

    sLog->outString("Starting CrystalShard RA on port %d on %s", raport, stringip.c_str());
    ...
    sLog->outStaticDebug("CrystalShard RA thread exiting");
}
